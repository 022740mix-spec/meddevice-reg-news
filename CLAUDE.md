...
11. **MDSAP参加形態の正確な区別**: 参加国（Regulatory Authority: USA/CAN/AUS/BRA/JPNの5カ国のみ）、Affiliate Member、Observer、非参加を正確に区別する
12. **他国の規制を別の国に適用しない**: Brexit後のGBRにEU MDRを適用する等の混同を避ける。各国の現行法制に基づいて記述する
13. **primaryLaw.title は正式名称のみ**: 和文説明・略称注記は description に分離する
14. **lastAmended は改正公布日**: 施行日ではない。施行日は description で補足する
